...
% This example simulates a simple cross flexure rotating from -0.25 to +0.25 radians
...
nprops(3).mominertia        = [1 0 0 1 0 1];    %Inertia [kg*m^2] of node 3 in order Ixx, Ixy, Ixz, Iyy, Iyz, Izz
...
eprops(1).nbeams   = 4;                %4 beam elements for simulating these elements
eprops(1).flex     = [3 4];        	   %Model out-of-plane bending (modes 3 and 4) as flexible
...
%rls = [];                             %Create empty rls variable to check for overconstraints
